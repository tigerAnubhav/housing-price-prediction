...
class FeatureAnalysisMixin:
    ...
    def variable_summary(self, cols=None):
        ...
        for col in cols:
            ...
            vs.loc[col, SUMMARY_KEY_MAP.samples] = compute_if_dask(
    ...
    def non_numeric_frequency_plot(
        ...
        if cols:
            cols = [x for x in cols if x in self.get_non_numeric_columns()]
        else:
            cols = self.get_non_numeric_columns()
        plot = FrequencyPlot(self.data).get_plots(cols)
        if save_plots:
            save_plot(plot, file_path)
        if return_plots:
    ...
    def numeric_distributions(self, cols=None):
        ...
        if not cols:
            cols = self.get_numeric_columns()
        from . import PercentilePlot, FrequencyPlot

        density = FrequencyPlot(self.data)
        percentile = PercentilePlot(self.data)

        def get_distribution(col):
            return (density.get_plots([col]) + percentile.get_plots([col])).cols(1)
    ...
    def feature_distributions(self, cols=None):
        ...
        if self.get_numeric_columns():
            num_result = self.numeric_distributions(self.get_numeric_columns())
        ...
        if self.get_non_numeric_columns():
            ...
            ] = get_plot_dict_for_dynamicmap(self.non_numeric_frequency_plot())
        else:
            self.feature_distributions_result[
                "non_numeric_variables"
    ...
    # @fail_gracefully(_LOGGER)
    @measure_time(_LOGGER)
    def feature_analysis(self, cols=[], save_as=None, save_path="", type="both"):
        ...
        if type in ["raw", "both"]:
            ...
            report["summary_stats"]["numeric_variables"] = [
                self.numeric_summary(cols=cols)
            ]

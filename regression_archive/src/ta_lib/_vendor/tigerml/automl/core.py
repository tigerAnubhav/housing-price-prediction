...
class AutoML:
    ...
    **kwargs: key-words arguments
    ...
    def get_report(self, no_of_pipelines=10, format=".html", reports_path=""):
        ...
        top_pipelines_str = "top_" + (str(no_of_pipelines) + "_pipelines_metrics")
        ...
        report_dict[top_pipelines_str].update({"": metrics_formatted})
        if conf_matrices is not None:
            ...
            report_dict["Top_" + str(no_of_pipelines) + "_Pipelines_Conf_Matrices"] = {
                "": conf_matrices_formatted
            }
